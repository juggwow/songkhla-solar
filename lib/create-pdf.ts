...
export async function generateConfirmationPDF(caQoute:CAQoute) {
    ...
    const nameAddressBusiness = `\t\t ข้าพเจ้า ${caQoute.customer.name} ${mockBusiness} ที่อยู่ ${caQoute.customer.address}` + 
                        `มีความประสงค์ให้การไฟฟ้าส่วนภูมภาคสาขาเมืองสงขลา จัดส่งเจ้าหน้าที่ดำเนินการดังนี้ `;
    const nameAddressBusinessHeiht = heightFont16*Math.ceil(customFont.widthOfTextAtSize(nameAddressBusiness,16)/maxWidth)
